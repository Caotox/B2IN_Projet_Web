...
// logger les réponses du serveur (voir console jsp si ça va marcher)
app.use((req, res, next) => {
  const originalSend = res.send;
  res.send = function (data) {
    console.log('Réponse serveur : ', data);  // log les réponses du serveur inchallah
...
app.post('/api/inscription', async (req, res) => {
  ...
  try {
    ...
    // const hashedPassword = await bcrypt.hash(mot_de_passe, 10);  // Pour le hachage
